...
int main() {
    string part_path = "../calibrateCamera/images/";
    ifstream inImgPath(part_path + "calibdata.txt");    //标定所用图像文件的路径
    ...
    ofstream fout(part_path + "caliberation_result.txt");   //保存标定结果的文件

    cout << "开始提取角点......" << endl;
    cv::Size image_size;//保存图片大小
    cv::Size pattern_size = cv::Size(9, 6);//标定板上每行、每列的角点数；测试图片中的标定板上内角点数为4*6
    ...
    while (image_num < imgList.size()) {
        ...
        cv::Mat imageInput = cv::imread(part_path + filename);
        if (image_num == 1) {
        ...
        if (findChessboardCorners(imageInput, pattern_size, corner_points_buf) == 0) {
            ...
            cv::Mat gray, gray_resize;
            cv::cvtColor(imageInput, gray, COLOR_RGB2GRAY);
            ...
            cv::resize(gray, gray_resize, Size(gray.cols / 2, gray.rows / 2));
            cv::imshow("camera calibration", gray_resize);
            cv::waitKey(50);
        }
    }
    ...
    cv::calibrateCamera(objectPoints, corner_points_of_all_imgs, image_size, cameraMatrix, distCoefficients, rvecsMat,
                        tvecsMat, 0);
    cout << "标定完成" << endl;
    ...
    for (i = 0; i < image_num; i++) {
        ...
        cv::projectPoints(tempPointSet, rvecsMat[i], tvecsMat[i], cameraMatrix, distCoefficients,
        ...
        for (j = 0; j < tempPointSet.size(); j++) {
            image_points_calculated_mat.at<cv::Vec2f>(0, j) = cv::Vec2f(image_points_calculated[j].x,
                                                                        image_points_calculated[j].y);
    ...
    for (int i = 0; i < image_num; i++) {
        ...
        cv::Mat src_image = cv::imread(part_path + imgList[i], 1);
        cv::Mat new_image = src_image.clone(), src_image_resize, new_image_resize;
        cv::remap(src_image, new_image, mapx, mapy, cv::INTER_LINEAR);
        cv::resize(src_image, src_image_resize, Size(src_image.cols / 2, src_image.rows / 2));
        cv::resize(new_image, new_image_resize, Size(new_image.cols / 2, new_image.rows / 2));
        imshow("原始图像", src_image_resize);
        imshow("矫正后图像", new_image_resize);
        ...
        cv::imwrite("../calibrateCamera/stored_images/" + imageFileName, new_image);
